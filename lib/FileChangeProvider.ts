...
const fileChangeCollapsedStateMap = {};

export const connectFileChangeTreeView = (view: TreeView<KpmItem>) => {
    return Disposable.from(
        view.onDidCollapseElement(async e => {
            const item: KpmItem = e.element;
            fileChangeCollapsedStateMap[item.label] =
...
export class FileChangeProvider implements TreeDataProvider<KpmItem> {
    ...
    getTreeItem(p: KpmItem): KpmTreeItem {
        ...
        if (p.children.length) {
            let collasibleState = fileChangeCollapsedStateMap[p.label];
            if (!collasibleState) {
    ...
    async getChildren(element?: KpmItem): Promise<KpmItem[]> {
        ...
        if (element) {
            ...
            kpmItems = await kpmProviderMgr.getFileChangeTreeParents();
        }
        return kpmItems;
    }
}
